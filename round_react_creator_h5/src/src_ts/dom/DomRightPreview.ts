...
import UtilWebgl from "../webgl/UtilWebgl";
import WebglCtxProps from "../webgl/WebglCtxProps";
import WebglRSOrigin from "../webgl/WebglRSOrigin";
import WebglRSType from "../webgl/WebglRSType";
...
class DomRightPreview extends NodeModules.react.Component {
     ...
     * rt - 抗锯齿前
     */
    screenRT: WebGLTexture;
    /**
     * 帧缓冲区 - 抗锯齿前
     */
    screenFbo: WebGLFramebuffer;
    /**
     * 着色程序，相关属性名规范：rr 开头
     */
    ...
    /**
     * webgl 程序 - 绘制单一图片，相关属性名规范：img 开头
     */
    ...
    componentDidMount () {
        this.webglCanvas = this.webglCanvasRef.current as any;
        this.webglCtx = this.webglCanvas.getContext (`webgl`);

        this.d2Canvas = this.d2CanvasRef.current as any;
...
`
    ...
    private _float32Array = new Float32Array (1);
    /**
     * 获取尺寸够 length 的 32 位浮点数组
     * @param length 
     * @returns 
     */
    getFloat32Array (length: number) {
        // 长度不够的话，扩容到够位置
    ...
    private _uint8Array = new Uint8Array (1);
    /**
     * 获取尺寸够 length 的 32 位浮点数组
     * @param length 
     * @returns 
     ...
     * @param h 
     */
    get2DImgData (w: number, h: number) {
